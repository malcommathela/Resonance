    ...
    const assistantMessage = await prisma.$transaction(async (tx) => {
      const msg = await createMessageWithSequence(tx, {
        ...
        metadata: {
          ...
          designVersion,
          simulationVersion: context.versions?.simulationVersion ?? null,
          reportVersion: context.versions?.reportVersion ?? null,
          contextHealth: context.contextHealth ?? null,
          contextRevision: context.designContext?.contextRevision ?? null,
